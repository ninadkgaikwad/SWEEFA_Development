...
% Last Modified by GUIDE v2.5 12-Mar-2018 11:05:52
...
set(handles.ComputationOptionsMenu,'Enable','off');
set(handles.ComputationOptionsMenu,'Value',1);
...
set(handles.LpYear,'Enable','on');
set(handles.NLpYear,'Enable','on');
...
set(handles.Sim1,'Enable','on');
set(handles.Sim2,'Enable','on');
...
% Getting Data from Wind_GUI_MainPage based on Radio Button Values
...
elseif (ForecastMode==1) % Forecast Mode is Selected
    ...
    WindSimResults_FullPath=pwd; % Temporary
    setappdata(Wind_GUI_MainPage,'WindSimResults_FullPath',WindSimResults_FullPath);
    ...
    ComputationOptionsMenu=get(handles.ComputationOptionsMenu,'Value'); % Computation Options Menu

    WeatherFile=getappdata(Wind_GUI_MainPage,'TempFile');

    % Combining All User Input from Wind_GUI_MainPage in a Single Cell

    SimParameters={PrjName,PlantCapacity,Res,ComputationOptionsMenu,WeatherFile};

    % Getting Data from Wind_Loss_Parameters_GUI

    WindLossParameters=getappdata(Wind_Loss_Parameters_GUI,'WindLossParameters');
    
    % Getting Lat-Long-Alt Edit Boxes Handles from Wind_GUI_MainPage
    
    LatLongAlt_Pant_Handles={handles.Lat,handles.Long,handles.Altitude};

    % Using External Function for Energy Estimation of the Wind Power Plant

    [EnergySimulation_Status] = WTGEnergySimulation_ForecastMode( SimParameters,AllParameters,WindLossParameters,LatLongAlt_Pant_Handles );
    
    % Opening Simulation Completion GUI:

    Wind_App_Simulation_Completion_GUI;
...
% Getting Normal and Forecast Mode Radio Button Values

NormalMode=get(handles.NormalMode,'Value');
ForecastMode=get(handles.ForecastMode,'Value');

% Putting Normal and Forecast Mode Radio Button Values in Current GUI Workspace

setappdata(Wind_GUI_MainPage,'NormalMode',NormalMode);
setappdata(Wind_GUI_MainPage,'ForecastMode',ForecastMode);
...
% Sending Computation Option into Current GUI Workspace for Forecast Mode
if (ForecastMode==1)
    
    ComputationOptionsMenu=get(handles.ComputationOptionsMenu,'Value');
    
    setappdata(Wind_GUI_MainPage,'ComputationOptionsMenu',ComputationOptionsMenu);
    
end

% Putting Above Variables in the Current GUI Workspace

setappdata(Wind_GUI_MainPage,'NormalMode',NormalMode);
setappdata(Wind_GUI_MainPage,'ForecastMode',ForecastMode);

% Opening Wind Plant Data Acquisition Application
...
% Getting Weather File Full Path:
[filename,pathname]=uigetfile({'*.*'},'Wind Plant Weather File Selector');
fullpathname=strcat(pathname, filename);

% Getting Pop-Up Menu Values

ComputationOptionsMenu=get(handles.ComputationOptionsMenu,'Value');

% Loading WeatherFile in the Simulation according to Pop-Up Menu Value

if ((ComputationOptionsMenu==2)||(ComputationOptionsMenu==3)) % Load Turbine-Wise Weather File   
    ...
    % Disintegrating Parameters_T Cells for subnum_T

    subnum_T1=Parameters_T1{1,2} ;
    subnum_T1=sum(subnum_T1);

    subnum_T2=Parameters_T2{1,2} ;
    subnum_T2=sum(subnum_T2);

    subnum_T3=Parameters_T3{1,2} ;
    subnum_T3=sum(subnum_T3);

    subnum_T4=Parameters_T4{1,2} ;
    subnum_T4=sum(subnum_T4);    
    
    % Getting Total Number of Turbines   
    
    subnum_Total=subnum_T1+subnum_T2+subnum_T3+subnum_T4;
    
    % Storing the Weather File contents to a Cell 
    
    for i=1:subnum_Total
        
        TempFile=xlsread(fullpathname,i);

        WeatherFile(1,i)  = {TempFile};        
        
    end       
    
elseif (ComputationOptionsMenu==4) % Load Plant WeatherFile
    ...
    WG_TurbineType_Total=sum(WG_TurbineType);
    
    % Storing the Weather File contents to a Cell 
    
    for i=1:WG_TurbineType_Total
...
setappdata(Wind_GUI_MainPage,'WeatherFile',WeatherFile);



% --- Executes on selection change in ComputationOptionsMenu.
function ComputationOptionsMenu_Callback(hObject, eventdata, handles)
% hObject    handle to ComputationOptionsMenu (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: contents = cellstr(get(hObject,'String')) returns ComputationOptionsMenu contents as cell array
%        contents{get(hObject,'Value')} returns selected item from ComputationOptionsMenu


% --- Executes during object creation, after setting all properties.
function ComputationOptionsMenu_CreateFcn(hObject, eventdata, handles)
% hObject    handle to ComputationOptionsMenu (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% Changing GUI elements behaviour according to Radio Button Values
if (NormalMode==1) % Normal Mode is selected
    ...
    set(handles.Altitude,'Enable','on');        
...
elseif (ForecastMode==1) % Forecast Mode is selected
    ...
    set(handles.LpYear,'Enable','off');
    set(handles.NLpYear,'Enable','off');
    ...
    set(handles.Sim1,'Enable','off');
    set(handles.Sim2,'Enable','off');      
