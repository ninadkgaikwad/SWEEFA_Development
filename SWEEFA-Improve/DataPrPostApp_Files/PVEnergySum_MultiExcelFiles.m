function [ ProcessedData ] = PVEnergySum_MultiExcelFiles( FileTypeNum,DataSeriesNumVector )
...
for i=ExcelFileRead;
    ...
    % Intializing DataAcquired
    data=xlsread(Fullpathname,1);
    [rr,cc]=size(data);
    DataAcquired=zeros(rr,cc,numOfSheets(1,i));
    
    clear data;
          
    for j=1:numOfSheets(1,i)
        
        Data1=xlsread(Fullpathname,j);
        
        [DataAcquired(:,:,j)]=Data1;
...
%% Calling External Function for Summation

Len1=length(ExcelFileData);

i=0; % Initializations

for i=1:Len1
    
    TypeVectorIndex=ExcelFileRead(1,i);

    [ SummationDataMatrix ] = PVEnergySummationOfExcelFiles_1(ExcelFileData{1,i}, TypeVector(1,TypeVectorIndex) , DataSeriesNumVector);

    ProcessedData{1,i}=SummationDataMatrix;
