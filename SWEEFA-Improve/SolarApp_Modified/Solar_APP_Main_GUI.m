...
% Last Modified by GUIDE v2.5 21-Feb-2018 15:24:56
...
set(handles.RegionalTime,'Value',0);
set(handles.LocalTime,'Value',0);
...
set(handles.GHICal,'Enable','on');
set(handles.UserGHI,'Enable','on');
set(handles.UserInso,'Enable','on');
...
set(handles.HourWise,'Enable','on');
set(handles.ResWise,'Enable','on');
set(handles.PB8,'Enable','on');
set(handles.PB9,'Enable','on');

set(handles.RegionalTime,'Enable','off');
set(handles.LocalTime,'Enable','off');

set(handles.uipanel21,'Visible','on');

set(handles.uibuttongroup6,'Visible','off');
set(handles.ForecastedWeatherFile,'Enable','off');
...
% Getting the Mode [Normal, Forecast] from Radio Button Values
NormalMode=get(handles.NormalMode,'Value');
ForecastMode=get(handles.ForecastMode,'Value');

% Changing Start Simulation Button behaviour according to the Radio Button Values
...
elseif (ForecastMode==1) % Forecast Mode is Selected
    ...
    % Getting Result Folder Path
    SolarSimResults_FullPath=pwd; % Temporary
    setappdata(Solar_APP_Main_GUI,'SolarSimResults_FullPath',SolarSimResults_FullPath);
    ...
    setappdata(Solar_APP_Main_GUI,'Alt',Alt);
    Alt=str2double(Alt);

    UTCTimeDiff=get(handles.UTCTimeDiff,'String');
    setappdata(Solar_APP_Main_GUI,'UTCTimeDiff',UTCTimeDiff);
    UTCTimeDiff=strread(UTCTimeDiff,'%f','delimiter',',');
    UTCTimeDiff=UTCTimeDiff';    
        
    RegionalTimeMeridian=UTCTimeDiff(1,2);
    UTCTimeDiff=UTCTimeDiff(1,1);
    ...
    %% Getting User Data Simulation Time-Step from the Step 5 (Temperature File & Wind Speed File)
    
    HHres=get(handles.HHres,'String');
    HHres=str2num(HHres);  
    
    WeatherFile=getappdata(Solar_APP_Main_GUI,'WeatherFile');
    
    % Getting Type of Date-Time Stamp of Weather File from Radio Button Values
    RegionalTime=get(handles.RegionalTime,'Value');
    LocalTime=get(handles.LocalTime,'Value');
    
    % Setting up Index according to Radio Button Values
    if (RegionalTime==1) % The Date-Time Stamp in the Weatherfile is Regional i.e. not Solar Time
        
        DateTime_Index=1;
        
    elseif (LocalTime==1) % The Date-Time Stamp in the Weatherfile is Local Solar Time
    ...
    %% Using External Function to evaluate Energy
    [ EnergySimulation_Status ] = SolarPVEnergySimulation_ForecastMode( PrjName,Lat,Long,Alt,UTCTimeDiff,Pmod,PVTech,ModNum,ModTemCF,Tn,Gn,SF,LID,LS,Arraymismat,OhmicLoss,rho,TrackerL,INVeff,TransLoss,Uo,U1,Shading,bo,WeatherFile,OrientationType,OrientationParameters,HHres,PlantCapacity,DateTime_Indexm,SolarSimResults_FullPath,RegionalTimeMeridian);
...
%     Solar_App_FinalReport_GUI; 
    ...
    % Opening Simulation Completion GUI:

    Solar_App_Simulation_Completion_GUI;
...
% File Name can be created from User Data
% file = strcat(PrjName,'_','ConfigFile')
...
% --- Executes on button press in ForecastMode.
function ForecastMode_Callback(hObject, eventdata, handles)
% hObject    handle to ForecastMode (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of ForecastMode
...
% Changing GUI behaviour according to the Radio Button Values
...
elseif (ForecastMode==1) % Forecast Mode is selected
    ...
    set(handles.GHICal,'Enable','off');
    set(handles.UserGHI,'Enable','off');
    set(handles.UserInso,'Enable','off');    
    ...
    set(handles.HourWise,'Enable','off');
    set(handles.ResWise,'Enable','off');
    set(handles.PB8,'Enable','off');
    set(handles.PB9,'Enable','off');
    
    set(handles.RegionalTime,'Value',0);
    set(handles.LocalTime,'Value',0);

    set(handles.RegionalTime,'Enable','on');
    set(handles.LocalTime,'Enable','on');    

    set(handles.uipanel21,'Visible','off');
    
    set(handles.uibuttongroup6,'Visible','on');
    set(handles.ForecastedWeatherFile,'Enable','on'); 
...
% --- Executes on button press in ForecastedWeatherFile.
function ForecastedWeatherFile_Callback(hObject, eventdata, handles)
% hObject    handle to ForecastedWeatherFile (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
% Loading Temperature File into Simulation
[filename,pathname]=uigetfile({'*.*'},'Solar Plant Weather File Selector');
fullpathname=strcat(pathname, filename);
TempFile=xlsread(fullpathname);
setappdata(Solar_APP_Main_GUI,'WeatherFile',TempFile);
