function [ DTGeneration_Cleaned_Cell ] = WTGGenerationDataCleaner_NormalMode( DTGeneration_Cell,DTWeather_Cleaned_Cell,Res,N,GenIrrad,DataCumulativeOrNot,WTGGenCleanAlgorithm,AllParameters,WindLossParameters,SheetNum,SheetType   )
...
if (WTGGenCleanAlgorithm==1) % N-Point Average Method
    ...
    for i=1:rrnum % For Each Row in ProcessedData
        ...
        if (isnan(ProcessedData(i,end))) % Checking for Missing Data           
            ...
            [WTG_Generation]=WTGEnergySimulation_PointWise_NormalMode(WS(i,1),Temp(i,1),Res,GenIrrad,AllParameters_Disintegrated_Cell,WindLossParameters,WTGGenCleanAlgorithm);
