...
% Last Modified by GUIDE v2.5 09-Mar-2018 16:02:50
...
set(handles.UTC,'Value',1);
set(handles.Regional,'Value',0);
set(handles.UTC,'Enable','off');
set(handles.Regional,'Enable','off');

set(handles.WindPlant,'Value',1);
set(handles.SolarPlant,'Value',0);
set(handles.WindPlant,'Enable','off');
set(handles.SolarPlant,'Enable','off');

set(handles.UTCTimeDiff,'String','');
set(handles.AdditionalFields,'String','');
set(handles.UTCTimeDiff,'Enable','off');
set(handles.AdditionalFields,'Enable','off');

set(handles.Lat1,'String','');
set(handles.Long1,'String','');
set(handles.Alt,'String','');
set(handles.Lat1,'Enable','off');
set(handles.Long1,'Enable','off');
set(handles.Alt,'Enable','off');

set(handles.LatLongFile,'Enable','off');
%set(handles.AltFile,'Enable','off');

set(handles.uibuttongroup8,'Visible','on');

set(handles.uipanel17,'Title','Wind Plant Site Info')
...
set(handles.r7,'Enable','on');   

set(handles.b2,'Enable','on');

set(handles.b3,'Enable','on');

set(handles.Netcdf_Variable,'Enable','on');
...
if (r3==1) % Plot 2D Variable
    ...
    Long=get(handles.Long1,'String');
    Lat=get(handles.Lat1,'String');
...
% Getting the Fullpath of the File Folder where the Extracted Files will be saved
WRFNetcdfExtractedFiles_FullPath=pwd;
setappdata(NETCDF_TEST_GUI,'WRFNetcdfExtractedFiles_FullPath',WRFNetcdfExtractedFiles_FullPath)

% Getting Mode [Normal, Forecast] Radio Button Values
NormalMode = get(handles.NormalMode, 'Value');
ForecastMode = get(handles.ForecastMode, 'Value');

% Changing Extract Button behaviour according to the Radio Button Values
...
elseif (ForecastMode==1) % Forecast Mode is Selected
    ...
    % Getting the Mode for Type of Date-Time Stamp from Radio Button Values
    UTC = get(handles.UTC,'Value');
    Regional = get(handles.Regional,'Value');
    
    % Setting up Indices according to Radio Button Values
    if (UTC==1) % UTC Date-Time Stamp is Selected
    ...
    elseif (Regional==1) % Regional Date-Time Stamp is Selected
        
        % Setting up Index
        DateTime_Index=2;        

        % Getting the Appropriate Data
        UTCTimeDiff=get(handles.UTCTimeDiff,'String');
        UTCTimeDiff=str2num(UTCTimeDiff);        
        
    end  
    
    % Getting Other Data Fields   
    AdditionalFields=get(handles.AdditionalFields,'String');
    
    % Getting the Power Plant Mode [Wind, Solar] from Radio Button Values
    WindPlant = get(handles.WindPlant,'Value');
    SolarPlant = get(handles.SolarPlant,'Value');
    
    % Setting up Indices and getting appropriate Data according to the Radio Button Values 
    if (WindPlant==1) % Wind Plant is Selected
        
        % Setting up Plant Index
        Plant_Index=1;
        
        % Getting Appropriate Data
        LatLongAltFile=getappdata(NETCDF_TEST_GUI,'Wind_LatLongAlt_File');
        %AltFile=getappdata(NETCDF_TEST_GUI,'Wind_Altitude_File');
        
        % Dividing LatLongAltFile into LatLongFile and AltFile
        LatLongFile=LatLongAltFile(:,1:2);
        AltFile=LatLongAltFile(:,3);
    ...
    elseif (SolarPlant==1) % Solar Plant is Selected
        ...
        Long1=get(handles.Long1, 'String');
        Long1=str2num(Long1);
        
        % Creating Lat-Long File
        
        LatLongFile=[Lat1, Long1];
        
        Alt=get(handles.Alt, 'String');
        AltFile=str2num(Alt);        
        
    end
    
    Netcdf_Total_GUI_Var=handles.Netcdf_Total;
    
    % Calling External Function to Extract and Post-Process NETCDF Variables in Excel Files
    [ProcessedData]=Extract_PostProcess_NETCDFVariables(Fullpathname, DateTime_Index, Plant_Index, UTCTimeDiff, AdditionalFields, LatLongFile, AltFile,Netcdf_Total_GUI_Var);
    
    % Opening the WRF NETCDF Extraction Completion GUI
    NETCDF_App_Extraction_Completion_GUI;
...
if (r3==1)
    ...
    set(handles.Long1,'Enable','on');
    set(handles.Lat1,'Enable','on');
...
function UTCTimeDiff_Callback(hObject, eventdata, handles)
% hObject    handle to UTCTimeDiff (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of UTCTimeDiff as text
%        str2double(get(hObject,'String')) returns contents of UTCTimeDiff as a double


% --- Executes during object creation, after setting all properties.
function UTCTimeDiff_CreateFcn(hObject, eventdata, handles)
% hObject    handle to UTCTimeDiff (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --- Executes on button press in UTC.
function UTC_Callback(hObject, eventdata, handles)
% hObject    handle to UTC (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of UTC


% --- Executes on button press in Regional.
function Regional_Callback(hObject, eventdata, handles)
% hObject    handle to Regional (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of Regional



function AdditionalFields_Callback(hObject, eventdata, handles)
% hObject    handle to AdditionalFields (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of AdditionalFields as text
%        str2double(get(hObject,'String')) returns contents of AdditionalFields as a double


% --- Executes during object creation, after setting all properties.
function AdditionalFields_CreateFcn(hObject, eventdata, handles)
% hObject    handle to AdditionalFields (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --- Executes on button press in LatLongFile.
function LatLongFile_Callback(hObject, eventdata, handles)
% hObject    handle to LatLongFile (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
[filename,pathname]=uigetfile({'*.*'},'Wind Plant Lat-Long File Selector');
fullpathname=strcat(pathname, filename);
TempFile=xlsread(fullpathname);
setappdata(NETCDF_TEST_GUI,'Wind_LatLongAlt_File',TempFile);

% --- Executes on button press in AltFile.
function AltFile_Callback(hObject, eventdata, handles)
% hObject    handle to AltFile (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
% Loading Temperature File into Simulation
[filename,pathname]=uigetfile({'*.*'},'Wind Plant Altitude File Selector');
fullpathname=strcat(pathname, filename);
TempFile=xlsread(fullpathname);
setappdata(NETCDF_TEST_GUI,'Wind_Altitude_File',TempFile);
...
% Changing GUI behaviour according to Radio Button Values

if (NormalMode==1) % Normal Mode is Selected
    
    set(handles.UTC,'Value',0);
    set(handles.Regional,'Value',0);
    set(handles.UTC,'Enable','off');
    set(handles.Regional,'Enable','off');
    ...
    set(handles.Netcdf_Variable,'Enable','on');  
    set(handles.Netcdf_Total,'Enable','on'); 
...
elseif (ForecastMode==1) % Forecast Mode is Selected
    ...
    set(handles.UTC,'Value',1);
    set(handles.Regional,'Value',0);
    set(handles.UTC,'Enable','on');
    set(handles.Regional,'Enable','on');

    set(handles.WindPlant,'Value',1);
    set(handles.SolarPlant,'Value',0);
    set(handles.WindPlant,'Enable','on');
    set(handles.SolarPlant,'Enable','on');

    set(handles.UTCTimeDiff,'String','');
    set(handles.AdditionalFields,'String','');
    set(handles.UTCTimeDiff,'Enable','off');
    set(handles.AdditionalFields,'Enable','on');
    ...
    set(handles.LatLongFile,'Enable','on');
    %set(handles.AltFile,'Enable','on');
...
if (UTC==1)  % UTC Option is Selected
    
    set(handles.UTCTimeDiff,'Enable','off');
    
elseif (Regional==1) % Regional Option is Selected
    
    set(handles.UTCTimeDiff,'Enable','on');
...
if (WindPlant==1) % Wind Plant Option is Selected
...
elseif (SolarPlant==1) % Solar Plant Option is Selected
    
    set(handles.uipanel17,'Title','Solar Plant Site Info')
