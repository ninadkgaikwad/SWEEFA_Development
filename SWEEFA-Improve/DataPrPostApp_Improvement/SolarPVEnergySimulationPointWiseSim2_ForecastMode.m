function [ ProcessedData, PgridEnergy,Irradiance] = SolarPVEnergySimulationPointWiseSim2_ForecastMode(ProcessedData,GenIrrad,WeatherFileHourlyOrRes,n,Index,Day,Col,Lat,Pmod,PVTech,ModNum,ModTemCF,SF,LID,LS,Arraymismat,OhmicLoss,rho,INVeff,TransLoss,Uo,U1,Shading,bo,Temp,WS,OrientationType,OrientationParameters,Years,RainSnowMonths,yr,mr,YearRain,MonthRain,HHres,SheetType) 
...
if ((GenIrrad(1,1)==1)||(GenIrrad(1,1)==2)) % Data Column is Generation Data  
        ...
        for jj=n
            ...
            for ii=CorrectedIndex
                ...
                % User Defined Time-Step Level Simulation                
                    ...
                    if (SheetType==1) % One Sheet for each PVTechs
                    ...
                    elseif (SheetType==2) % One Sheet for All PVTechs
                        
                        [ Pmodtot,Pmodin]=ModulePower( Pmod,ModTemCF,ModNum(1,PVParaIndex),Tn,Gn,Icsf,T(hi,1),Ic,Uo,U1,Ws(hi,1) );
                    ...
                    if(GenIrrad(1,1)==1) % Generation is in Energy (kWh)
                    ...
                    elseif (GenIrrad(1,1)==2) % Generation is in Power (kW)
                        
                        % Converting Summed Power To Energies (Energy *(HHres/60))
                        PVoutEnergy=PVoutPower;
                        INVpinEnergy=INVpinPower;
                        INVpoutEnergy=INVpoutPower;

                        PgridEnergy=PgridPower*EnergyConverter/1000;
                        ProcessedData(ii,(Col+4))=PgridEnergy; % Updating Generation Data in the Correct Column and Row

                        ArrayMismatchLossEnergy=ArrayMismatchLossPower;
                        ShadingLossEnergy=ShadingLossPower;
                        LIDLossEnergy=LIDLossPower;
                        OhmicLossPEnergy=OhmicLossPPower;
                        InverterLossEnergy=InverterLossPower;
                        TransformerLossPEnergy=TransformerLossPPower;                         
...
elseif GenIrrad(1,1)==0 % Data Column is Irradiance Data 
