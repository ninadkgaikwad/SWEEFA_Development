function [ ProcessedData,Z,N ] = SolarPVGenerationDataCleaner( Headers,Res,DataCols,hem,Ltm,L,Lat,GenCapacity,GenIrrad, WeatherFileHourlyOrRes, DataCumulativeOrNot, PlantInfo_Parameters,DateTimeString,NegativeCol,SolarRegionalTime )
...
% File Selection
[Filename,Pathname]=uigetfile({'*.*'},'Raw Data Solar Generation File Selector');
...
for i=1:TotDays
    ...
    for k=1:DataCols
        ...
        if length(NaNCheck1)==DayDataPoints % Use PV Energy Evaluation Model to fill missing DATA
...
filename = [Filename,'_ProcessedSolarGenFile.xlsx'];
